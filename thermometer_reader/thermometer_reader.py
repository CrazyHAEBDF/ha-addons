...
CALIBRATION_FILE = CONFIG_DIR / "thermometer_calibration.json"
TEMPLATE_FILE = CONFIG_DIR / "thermometer_cluster_template.jpg"


def log(msg):
    print(f"[thermometer] {msg}", flush=True)
...
def mqtt_client_from_options(opt):
    ...
    client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, client_id="ha_thermometer_reader", clean_session=True)
...
def thermometer_options(opt):
    return {
        "angle_min_deg": float(opt.get("angle_20_deg", 315.0)),
        "angle_max_deg": float(opt.get("angle_100_deg", 225.0)),
        "scale_min": 20.0,
        "scale_max": 100.0,
        "clockwise": True,
        "search_max_value": 100.0,
        "scan_inner_ratio": float(opt.get("scan_inner_ratio", 0.12)),
        "scan_outer_ratio": float(opt.get("scan_outer_ratio", 0.98)),
        "line_half_width_px": int(opt.get("line_half_width_px", 1)),
        "processing_scale": int(opt.get("processing_scale", 4)),
    }


def publish_thermometer_discovery(client, opt):
    base = opt["mqtt_base_topic"].rstrip("/")
    discovery = opt["mqtt_discovery_prefix"].rstrip("/")
    state_topic = f"{base}/state"
    avail_topic = f"{base}/availability"
    device = {
        "identifiers": ["heizung_thermometer_reader"],
        "name": "Heizungsthermometer",
        "manufacturer": "Custom",
        "model": "RTSP/OpenCV Thermometer Reader 1.0.0",
    }
    entities = []
    for key, oid, name in (
...
def annotate_thermometer_roi(roi, cx, cy, radius, result, value, valid, label):
    annotated = roi.copy()
    ...
    cv2.putText(annotated, f"{label} {value:.1f} C", (2, 13), cv2.FONT_HERSHEY_SIMPLEX, 0.32, (255,255,255), 1, cv2.LINE_AA)
...
def save_thermometer_debug(frame, cluster, candidates, return_data=None, supply_data=None, status=""):
    DEBUG_DIR.mkdir(parents=True, exist_ok=True)
    overview = frame.copy()
    for candidate in candidates or []:
        p = (int(round(candidate["cx"])), int(round(candidate["cy"])))
...
def thermometer_main():
    ...
    histories = {"return": deque(maxlen=int(opt.get("median_window", 5))), "supply": deque(maxlen=int(opt.get("median_window", 5)))}
    client = mqtt_client_from_options(opt)
    publish_thermometer_discovery(client, opt)
    base = opt["mqtt_base_topic"].rstrip("/")
    pending = None
    confirmations = 0
    last_locate = 0.0
    ...
    capture_failures = 0
    last_candidates = []
    log("MQTT verbunden; Rücklauf, Zulauf und Debug-Kameras veröffentlicht.")
    ...
    while True:
        ...
        try:
            ...
            capture_failures = 0
            now = time.time()
            must_locate = baseline_cluster is None or current_cluster is None or now-last_locate >= locate_interval or invalid_count >= 2
            ...
            if must_locate:
                ...
                if found:
                    ...
                    if baseline_cluster is None:
                        ...
                        log(f"Thermometer-Cluster bestätigt {confirmations}/{confirmations_required}")
                        if confirmations >= confirmations_required:
                            baseline_cluster = current_cluster = found
                            current = {"cx": m["cx"], "cy": m["cy"], "r": m["r"], "source": "cluster", "score": score}
                            baseline = current.copy()
                            save_calibration(baseline, current, baseline_cluster, current_cluster)
                            log("Thermometer-Referenz angelegt.")
                    else:
                        current_cluster = found
                        current = {"cx": m["cx"], "cy": m["cy"], "r": m["r"], "source": "cluster", "score": score}
                        save_calibration(baseline, current, baseline_cluster, current_cluster)
                    ...
                    current_cluster = None
                    confirmations = 0
                    log("WARNUNG: Thermometer-Cluster nicht gefunden.")

            if baseline_cluster is None or current_cluster is None:
                payload = {"return_temperature": None, "supply_temperature": None,
                           "return_valid": False, "supply_valid": False, "cluster_found": False,
                           "return_confidence": 0.0, "supply_confidence": 0.0,
                ...
                if bool(opt.get("debug", True)):
                    ov,_,_ = save_thermometer_debug(frame, current_cluster or pending, last_candidates, status=payload["status"])
                ...
                for key, item in (("return", current_cluster["t1"]), ("supply", current_cluster["t2"])):
                    ...
                    cx += float(opt.get("needle_center_x_ratio",0.0))*item["r"]
                    cy += float(opt.get("needle_center_y_ratio",-0.55))*item["r"]
                ...
                invalid_count = 0 if results["return"][2] and results["supply"][2] else invalid_count+1
                payload = {
                    "return_temperature": round(results["return"][0],2), "supply_temperature": round(results["supply"][0],2),
                    "return_raw": round(results["return"][1]["pressure_raw"],2), "supply_raw": round(results["supply"][1]["pressure_raw"],2),
                    "return_valid": results["return"][2], "supply_valid": results["supply"][2], "cluster_found": True,
                    "return_confidence": round(results["return"][1]["confidence"],3), "supply_confidence": round(results["supply"][1]["confidence"],3),
                ...
                if bool(opt.get("debug",True)):
                    ov,rb,sb=save_thermometer_debug(frame,current_cluster,last_candidates,debug_data["return"],debug_data["supply"],payload["status"])
                    client.publish(f"{base}/debug/frame",ov,qos=0,retain=True)
                    client.publish(f"{base}/debug/return",rb,qos=0,retain=True)
                    client.publish(f"{base}/debug/supply",sb,qos=0,retain=True)
                log(f"Rücklauf={results['return'][0]:.1f} °C (conf {results['return'][1]['confidence']:.2f}) | Zulauf={results['supply'][0]:.1f} °C (conf {results['supply'][1]['confidence']:.2f})")
            sleep_for = interval
        except Exception as exc:
            capture_failures += 1
            log(f"FEHLER: {exc}")
